#include "SPI25LC256.h"
#include <avr/io.h>

void initSPI(void) {
  DDRB |= (1 << DDB2);                        /* set SS output */
  PORTB |= (1 << PORTB2);       /* start off not selected (high) */

  DDRB |= (1 << DDB3);                   /* output on MOSI */
  PORTB|= (1 << PORTB4);                  /* pullup on MISO */
  DDRB |= (1 << DDB5);                      /* output on SCK */
